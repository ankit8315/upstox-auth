...
// ── Robust JSON extractor — handles truncated AI responses ────────────────────
function extractJSON(raw) {
  ...
  // Walk backwards from end, try increasingly shorter substrings
  ...
  const openArrays  = (partial.match(/\[/g) || []).length;
  const closeArrays = (partial.match(/\]/g) || []).length;
  ...
  for (let i = 0; i < openArrays - closeArrays; i++) patched += "]";
...
async function checkTheses(activeCalls, currentPrices, marketContext) {
  ...
  try {
    ...
    const parsed = extractJSON(raw);
    if (!parsed) throw new Error("No valid JSON in thesis response");
